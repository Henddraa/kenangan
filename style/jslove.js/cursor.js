/* ═══════════════════════════════════════════
   PINK SPARKLE CURSOR — JavaScript
   Hubungkan ke HTML:
   <script src="cursor.js"></script>
   Pastikan ada dua elemen ini di dalam <body>:
   <div id="cur-core"></div>
...
function spawnTrailAt(x, y) {
  ...
  el.style.cssText = `left:${x+rand(-5,5)}px;top:${y+rand(-5,5)}px;--life:${life};--r0:${r0};--r1:${r1};width:${sz}px;height:${sz}px;`;
...
document.addEventListener('mousemove', e => {
  if (core.style.display === 'none') core.style.display = 'block';
  mx = e.clientX;
  my = e.clientY;
  spawnTrailAt(mx, my);
});
...
document.addEventListener('click', e => {
  spawnLoveBurstAt(e.clientX, e.clientY);
...
document.addEventListener('touchstart', e => {
  ...
  touchDot.style.left = t.clientX+'px';
  touchDot.style.top  = t.clientY+'px';
  spawnLoveBurstAt(t.clientX, t.clientY);
  touchDot.style.boxShadow = '0 0 24px 10px #ff4da6,0 0 60px 20px rgba(255,77,166,.55)';
  touchDot.style.transform = 'translate(-50%,-50%) scale(2.2)';
  setTimeout(() => { touchDot.style.boxShadow=''; touchDot.style.transform=''; }, 280);
}, { passive: true });

document.addEventListener('touchend',    () => { touchDot.style.display='none'; });
document.addEventListener('touchcancel', () => { touchDot.style.display='none'; });